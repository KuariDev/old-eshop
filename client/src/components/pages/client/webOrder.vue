<template>
    <body>
        ...
        <div v-if="order" class="u-order">
            <h1 class="ent-ord">Благодарим за покупку!</h1>
            <p class="ur-prod">Ваш товар ({{ order.product?.title }}):</p>
            <div class="block-f"><textarea v-on:click="copyOrder(order.value)" class="copy-ord nokey" v-model="order.value" readonly></textarea>
            <p style="display: none; cursor: pointer;" v-bind:class="{ 'copied-block': !showCopied }" class='click-to-copy onkey' v-on:click="copyOrder(order.value)">Нажмите, чтобы скопировать</p>
            <p style="display: none" v-bind:class="{ 'copied-block': showCopied }" class='click-to-copy onkey' v-on:click="copyOrder(order.value)">Скопировано</p></div>
            <p class='click-to-copy onkey' style="text-align: center; color: black;">Обязательно запишите этот код получения. Если Вы случайно закроете страницу, то сможете получить лицензию  по этому коду получения, нажав на кнопку «активировать ключ</p>
            <button class="ord-btn" v-on:click="this.$router.push('/activate')">Активировать сейчас</button>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getOrder() {
            let orderCode = this.$route.params.orderData

            try {
                const response = await axios.get(`license/byCode/${orderCode}`)
